...
const countriesMap = new Set(countries.map((eleman) => [...eleman.languages]).flat());
console.log(countriesMap.size)

// 2

const mostSpokenLanguages = (array, number) => {
    const AllCountries = array.map((eleman) => eleman.languages).flat();

    const result = new Map();

    for (const language of AllCountries) {

        if (result.has(language)) {
            result.set(language, Number(result.get(language)) + 1);
        } else {
            result.set(language, 1);
        }
    }

    return new Map(Array.from(result.entries()).sort((a, b) => b[1] - a[1]).filter((eleman, i) =>
        i < number
    ));
}

console.log(mostSpokenLanguages(countries, 10))
const expectedMostSpokenLanguages = [{
    English: 91
}, {
    French: 45
}, {
